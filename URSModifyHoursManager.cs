...
namespace URS
{
    public partial class URSModifyHoursManager : Form
    {
        DAL dal = new DAL();
        ...
        public URSModifyHoursManager(URSOverviewManager _managerOverview, Manager _manager, int _ID)
        {
            ID = _ID;
            manager = _manager;
        ...
        private void URSModifyHoursManager_Load(object sender, EventArgs e)
        {
            Activities = dal.ShowActivities();
            locationComboBox.Items.Add("Thuis");
            locationComboBox.Items.Add("Kantoor");

            foreach (Hour hour in manager.Hours)
            {
        ...
        //Method to sync comboboxes to textboxes with help of a local database session
        ...
        {   
            ...
            {
                string message = manager.ModifyHours(Int32.Parse(updateHoursComboBox.Text), Int32.Parse(employeeIDTextBox.Text), Int32.Parse(activityIDComboBox.Text), dateTimePicker.Text, float.Parse(numberOfHoursTextBox.Text), locationComboBox.Text, descriptionTextBox.Text);
